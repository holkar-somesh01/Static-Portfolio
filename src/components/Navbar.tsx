...
//                 {theme === 'dark' ? <Sun size={20} /> : <Moon size={20} />}
//               </button>
//               <p className={`${theme === 'dark' ? "text-white" : "text-black"}`}>{theme === 'dark' ? "Light" : "Dark"}</p>
//             </div>
//           </div>
//         </div>
//       )}
...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed min-w-full bg-white/70 dark:bg-gray-900/70 backdrop-blur-md shadow-md z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="px-4 py-4 space-y-2">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                onClick={() => setIsOpen(false)}
                className="block text-gray-800 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 px-3 py-2 rounded-md text-base font-medium transition-colors"
              >
                {link.title}
              </a>
            ))}
